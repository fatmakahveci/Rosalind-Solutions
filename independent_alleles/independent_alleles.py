...
def independent_alleles(file_name):
	...
	for i in xrange(N,int(math.pow(2,k)+1)):
		sum+=(math.factorial(math.pow(2,k))/((math.factorial(math.pow(2,k)-i))*(math.factorial(i))))*math.pow(0.25,i)*math.pow(0.75,math.pow(2,k)-i)
	print sum
