...
file_name = 'mortal_fibonacci_rabbits.txt'
...
def get_variables(file_name):
    return list(map(int, read_file(file_name).split(' ')))
...
def survivor_rabbits(month, survival_time):
    ...
    for i in range(month):
        if i == 0 or i == 1:
            ...
            fibonacci_series[i] = fibonacci_series[i - 1] + fibonacci_series[i - 2] - fibonacci_series[
                i - survival_time - 1]
...
def main():
    print(str(mortal_fibonacci_rabbits(file_name)))
